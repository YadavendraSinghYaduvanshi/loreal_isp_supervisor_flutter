...
class OQADProductKnowledge extends StatefulWidget {
/*  String promoter_id;

  // In the constructor, require a Todo
  OQADProductKnowledge({Key key, @required this.promoter_id})
      : super(key: key);*/

  @override
  _OQADProductKnowledgeState createState() =>
      _OQADProductKnowledgeState();
}

class _OQADProductKnowledgeState
    extends State<OQADProductKnowledge> {
        ...
        body: Container(
          ...
          child: new Column(
            children: <Widget>[
              ...
              new Expanded(
                  child: FutureBuilder<List<OQAD_PRODUCT_KNOWLEDGE>>(
                    future: fetchData(),
  ...
  Future<List<OQAD_PRODUCT_KNOWLEDGE>> fetchData() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    //visit_date = prefs.getString('CURRENTDATE');
    user_id = prefs.getString('Userid');
    designation = prefs.getString('Designation');

    print("Attempting to fetch... OQAD_PRODUCT_KNOWLEDGE ");

    final url = "http://lipromo.parinaam.in/Webservice/Liwebservice.svc/GetAll";

    Map lMap = {
      "Downloadtype": "OQAD_PRODUCT_KNOWLEDGE",
      "Username": user_id,
      "per1": designation,
      "per2": user_id,
      "per3": "0",
...
List<OQAD_PRODUCT_KNOWLEDGE> parsePhotos(String responseBody) {
  ...
  var list = test1['OQAD_PRODUCT_KNOWLEDGE'] as List;
  statusList =
  list.map((i) => OQAD_PRODUCT_KNOWLEDGE.fromJson(i)).toList();
...
class OQAD_PRODUCT_KNOWLEDGE {
  final String PROMOTER;
  final int EMP_CD;
  final int TOTAL_ATTEMPTED;
  final int CORRECT_RESPONSE;

  OQAD_PRODUCT_KNOWLEDGE(
      {this.PROMOTER, this.EMP_CD, this.TOTAL_ATTEMPTED, this.CORRECT_RESPONSE,});

  factory OQAD_PRODUCT_KNOWLEDGE.fromJson(Map<String, dynamic> json) {
    return OQAD_PRODUCT_KNOWLEDGE(
...
class StatusList extends StatelessWidget {
  final List<OQAD_PRODUCT_KNOWLEDGE> status;
